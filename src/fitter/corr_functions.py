...
class CorrFunction:
    ...
    def two_h_conspire(cls, x, p):
        ...
        for n in range(x["n_state"]):
            for m in range(x["n_state"]):
                if n <= m:
                    En = cls.En({"state": x["denom"][0]}, p, n)
                    Em = cls.En({"state": x["denom"][1]}, p, m)
    ...
    @classmethod
    def eff_mass(cls, x, p, ax, color="k", alpha=0.4, tau=1, denom_x=None):
        xp = dict(x)
        xm = dict(x)
        ...
        if x["type"] in ["cosh", "cosh_const"]:
            if x["type"] == "cosh":
                corr = cls.cosh(x, p)
                corr_p = cls.cosh(xp, p)
                corr_m = cls.cosh(xm, p)
            else:
                corr = cls.cosh_const(x, p)
                corr_p = cls.cosh_const(xp, p)
                corr_m = cls.cosh_const(xm, p)
            ...
            if denom_x:
                ...
                if x["type"] == "exp_r":
                    corr = cls.two_h_ratio(x, p) / cls.exp(x0, p) / cls.exp(x1, p)
                    corr_p = cls.two_h_ratio(xp, p) / cls.exp(x0p, p) / cls.exp(x1p, p)
                elif x["type"] == "exp_r_conspire":
                    corr = cls.two_h_conspire(x, p) / cls.exp(x0, p) / cls.exp(x1, p)
                    corr_p = (
                        cls.two_h_conspire(xp, p) / cls.exp(x0p, p) / cls.exp(x1p, p)
                    )
                elif x["type"] == "exp_r_ind":
                    corr = cls.exp(x, p)
                    corr_p = cls.exp(xp, p)
            elif x["type"] == "exp_r":
                corr = cls.two_h_ratio(x, p)
                corr_p = cls.two_h_ratio(xp, p)
            elif x["type"] == "exp_r_conspire":
                corr = cls.two_h_conspire(x, p)
                corr_p = cls.two_h_conspire(xp, p)
...
class FitCorr(object):
    ...
    @classmethod
    def fit_function(cls, x, p):
        r = dict()
        for k in x:
            if x[k]["type"] == "exp":
                r[k] = cls.corr_functions.exp(x[k], p)
            elif x[k]["type"] == "cosh":
                r[k] = cls.corr_functions.cosh(x[k], p)
            elif x[k]["type"] == "cosh_const":
                r[k] = cls.corr_functions.cosh_const(x[k], p)
            elif x[k]["type"] == "exp_r_ind":
                r[k] = cls.corr_functions.exp(x[k], p)
            elif x[k]["type"] == "exp_r":
                sp = k.split("_")[-1]
                r[k] = cls.corr_functions.two_h_ratio(x[k], p)
                r[k] = r[k] / cls.corr_functions.exp(x[x[k]["denom"][0] + "_" + sp], p)
                r[k] = r[k] / cls.corr_functions.exp(x[x[k]["denom"][1] + "_" + sp], p)
            elif x[k]["type"] == "exp_r_conspire":
                sp = k.split("_")[-1]
                r[k] = cls.corr_functions.two_h_conspire(x[k], p)
                r[k] = r[k] / cls.corr_functions.exp(x[x[k]["denom"][0] + "_" + sp], p)
                r[k] = r[k] / cls.corr_functions.exp(x[x[k]["denom"][1] + "_" + sp], p)
        return r
...
def fcn(x, p):
    return FitCorr.fit_function(x, p)
