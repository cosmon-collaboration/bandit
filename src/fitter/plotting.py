...
def make_eff_plots(states, fp, x_fit, fit, gv_data, priors, scale, save_figs,show_fit ):
    ...
    if 'denom' in fp.corr_lst[k]:
        ...
        ax_r[k].axhspan(p.mean-p.sdev, p.mean
                        + p.sdev, color='k', alpha=.2)
        plot_eff(ax_r[k], gv_data, k, mtype=fp.corr_lst[k]['type'])
    ...
    ax_zeff[k] = plt.axes([0.15, 0.15, 0.84, 0.82]) # this raised deprecation warning; adding axes to same arg as prev axes
    ...
    plot_zeff(ax_zeff[k], gv_data, k, ztype=ztype,
...
def plot_eff_fit(states,fp,x_fit,fit,gv_data,priors,scale,save_figs=True):
    ax_meff = make_eff_plots(states, fp, priors, gv_data)
    print(ax_meff)
