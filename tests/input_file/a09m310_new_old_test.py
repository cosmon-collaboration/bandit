...
priors['pion_zS1_0'] = gv.gvar(4.7e-2, 5e-3)
priors['pion_zS2_0'] = gv.gvar(4.7e-3, 5e-4)
priors['pion_zP_0'] = gv.gvar(0.124,  0.015)
...
for corr in corr_lst:#[k for k in corr_lst if 'mres' not in k]:
    if 'mres' not in corr:
        for n in range(1,10):
            ...
            if n <= 2:
                priors['%s_zS1_%d' %(corr, n)] = gv.gvar(zS1_0.mean, 2*zS1_0.mean)
                priors['%s_zS2_%d' %(corr, n)] = gv.gvar(zS2_0.mean, 2*zS2_0.mean)
            else:
                priors['%s_zS1_%d' %(corr, n)] = gv.gvar(0, zS1_0.mean)
                priors['%s_zS2_%d' %(corr, n)] = gv.gvar(0, zS2_0.mean)

    # x-params
    N_src = len(corr_lst[corr]['srcs'])
